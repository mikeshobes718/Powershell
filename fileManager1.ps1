...
clear
Clear-Content C:\Users\u584422\Desktop\Script\Powershell\found2.txt

#The pid of our script
$pid_holder = $PID

#Get all PID's of "file name" and grep it like linux
$data = Get-WmiObject Win32_Process -Filter "Name='powershell.exe' AND CommandLine LIKE '%fileManager1.ps1%'" | Select-String -Pattern "Handle" -CaseSensitive


#Get count of times script is running
$count = (Get-WmiObject Win32_Process -Filter "Name='powershell.exe' AND CommandLine LIKE '%fileManager1.ps1%'").count
...
echo "Get-Content C:\Users\u584422\Desktop\Script\Powershell\found2.txt -Wait"
echo ""
...
$timeout = new-timespan -Minutes 21 #Instructions were ten minutes
...
while ($sw.elapsed -lt $timeout){
        ...
        function Display_File_Details($Fullname,$size,$mode,$psdate,$owner) { 
            ...
            $file_data | Out-File C:\Users\u584422\Desktop\Script\Powershell\found2.txt -append -noClobber
            }
        } 
...
echo "Will close window in 60 seconds or"
}elseif($minutes -gt 0){
clear
